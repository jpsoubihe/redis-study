...
public class AccountController {
    ...
    public ResponseEntity<String> createAccount(@RequestBody Account account) {
        ...
        metricsHelper.counter("sql.request.counter", "create_account");
        ...
        metricsHelper.timer("sql.request.timer", "create_account", Instant.now().toEpochMilli() - startTime);
    ...
    public ResponseEntity<Account> getAccount(@PathVariable(name = "id") String accountId) throws AccountNotFoundException {
        long startTime = Instant.now().toEpochMilli();
        metricsHelper.counter("sql.request.counter", "get_account");
        log.info("[GET] processing request for account with id={}", accountId);
        Account account = accountService.getAccount(accountId);
        metricsHelper.timer("sql.request.timer", "get_account", Instant.now().toEpochMilli() - startTime);
    ...
    public void updateAccount(@PathVariable(name = "id") String accountId, @RequestBody Account account) {
        long startTime = Instant.now().toEpochMilli();
        metricsHelper.counter("sql.request.counter", "update_account");
    ...
    public void deleteAccount(@PathVariable(name = "id") String accountId) {
        long startTime = Instant.now().toEpochMilli();
        metricsHelper.counter("sql.request.counter", "delete_account");
        log.info("[DELETE] processing request for account with id={}", accountId);
        accountService.deleteAccount(accountId);
        metricsHelper.timer("sql.request.timer", "delete_account", Instant.now().toEpochMilli() - startTime);
    }
}
